...
module.exports = (argv, options) => {
  ...
  if (hotswap) {
    log('Hot Swap mode active\n  targetJs: ' + targetJs + '\n  relativePathCompiledJs: ' + relativePathCompiledJs)
  } else {
    log(['Hot Swap mode NOT active.', outputArg, args.elmMakeArgs, relativePathCompiledJs].join('\n'))
  }
  ...
  const sendMessage = function (message) {
    if (webServer && webServer.sendMessage) {
      ...
      log('I could not sendMessage: ' + message)
    }
  }
...
`
  ...
  const build = () => {
    ...
    if (elmStderr) {
      try {
        var parsedError = JSON.parse(elmStderr)
...
`
        ...
        sendMessage(`compiling::${message}`)
        const buildResult = build()
        if (buildResult.exitCode === SUCCESS) {
          if (hotswap) {
            sendMessage(`hotswap::${relativePathCompiledJs}`)
          } else {
            sendMessage(`reload`)
          }
        } else {
          sendMessage(`error::${elmStderr}`)
        }

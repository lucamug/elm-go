...
console.log("\n")
paint(logo, "    ", [,,`  ${chalk.hex(color.c)('Elm Live')}`, `  ${chalk.dim('A flexible dev server for Elm')}`])
...
qrcode.toString('ciao',{type: 'utf8'}, function (err, string) {
  if (err) throw err
...
// const animation = [`._\\^`,`:-|~`,`'^/_`,`:-|~`]
// const animation = [`.:':.:':.:':.:':\\`,`:':.:':.:':.:':.|`,`':.:':.:':.:':.:/`, `:.:':.:':.:':.:'|`]
const animation = [`/_~^~_~^~ Com${chalk.dim('p')}ili${chalk.dim('n')}g! _~^~_~^~\\`
                  ,`|~^~_~^~_ Co${chalk.dim('m')}pil${chalk.dim('i')}ng! ~^~_~^~_|`
                  ,`\\^~_~^~_~ C${chalk.dim('o')}mpi${chalk.dim('l')}ing${chalk.dim('!')} ^~_~^~_~/`
                  ,`|~_~^~_~^ ${chalk.dim('C')}omp${chalk.dim('i')}lin${chalk.dim('g')}! ~_~^~_~^|`

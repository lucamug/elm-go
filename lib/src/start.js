...
function parseUrl (pathname, model) {
  ...
  if (pathname === '/reload/reload.js') {
    console.log(`::: INTERNAL ${pathname}`)
  } else if (fs.existsSync(fullPath)) {
    console.log(`::: OK ${pathname}`)
  } else {
    console.log(`::: NOT FOUND ${pathname} (pushstate: ${model.pushstate})`)
  }
  return {
    rootPath,
    fileType,
...
function handler (model) {
  ...
  return function (req, res) {
    ...
    // console.log("handler", {getPath: url.getPath, rootPath: url.rootPath, fileType: url.fileType});
